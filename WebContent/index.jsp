...
<script type="text/javascript">
	function doReport(sn){
	    ...
	    document.forms[0].action = '${pageContext.request.contextPath}/TBReport.do?sn='+sn;
	...
	function commit(){
	    ...
	    document.forms[0].action = '${pageContext.request.contextPath}/TBCover.do';
	    document.forms[0].submit();
	}
</script>
<%-- astar start --%>
...
<script type="text/javascript">
    ...
    function OpenIME() {      	
        OpenAstarIME('AstarIME.jsp', 'name');
    }
...
<STYLE TYPE="text/css">
	@font-face {
		...
		font-style:  normal;
		font-weight: normal;
		src: url("<c:url value='/Astar/BOTMing.eot'/>"); /* IE9 Compat Modes */
		src: /*url("<c:url value='/Astar/BOTMing.eot?#iefix'/>") format('embedded-opentype'),  IE6-IE8 */
			url("<c:url value='/Astar/BOTMing.ttf'/>") format('truetype');  /*Safari, Android, iOS */
	}
</STYLE>
<%-- astar end --%>
</head>
<body style="font-family: BOTFont;" onload="InitPage()">
<form name="f1" action="./TBCover.do" method="get" >
    <table border="1">
    <tr>
        <td style="text-align:right">輸入文字</td><td>
            <input type="text" id="name" name="name" value="" />
            <input type="hidden" id="sn" name="sn" value="" />
            <input type="button" value="開啟輸入視窗" onclick="OpenIME()" />
        </td>
    </tr>
     <tr>
    ...
    <c:if test="${list != null}">
		...
		<table border="1">
			<tr>
				...
				<td style="text-align:left">產生報表</td>
			</tr>
			<c:forEach var="vo" items="${list}">
				<tr>
					...
					<td style="text-align:left"><input type="button" name="bt1" value="產生報表" onclick="doReport('${vo.sn}');"> </td>
				</tr>
			</c:forEach> 
    	</table>
    </c:if>
